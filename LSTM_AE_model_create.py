...
fig.update_layout(title='source time series - '+timestr,xaxis_title="Time",yaxis_title='audio samples',showlegend=True)
...
my_model = LSTM_AE_model_delta1(X,X_train)
...
tf.keras.utils.plot_model(my_model, to_file=out_dir+timestr+'_my_modelplot.png', show_shapes=True, show_layer_names=True)
...
my_history = my_model.fit(X_train,y_train,epochs=model_parameters['my_epochs'],batch_size=model_parameters['my_batch_size'],validation_split=model_parameters['my_validation_split'],callbacks=[early_stop],shuffle=False)
...
fig.update_layout(title='Audio spectrum with anomalies - '+timestr,xaxis_title='Time',yaxis_title='Audio spectrum',showlegend=True)
...
fig.update_layout(title='training loss to epochs - '+timestr,xaxis_title='epochs',yaxis_title='loss',showlegend=True)
...
if USE_ANOTHERTESTFILE:
    nok_sequence = read_flac_to_pandas("visc6_ultrasonic_nok",start_sec=model_parameters['my_nok_startsec'], stop_sec=model_parameters['my_nok_stopsec'])
